...
<Quiz questions={[
  {
    question: "ベクトル要素数が32の倍数でない場合に発生する問題は何か？",
    options: [
      "コンパイルエラーが発生する",
      "最後のワープの一部スレッドがアイドル状態となりスレッド利用率が低下する",
      "GPUメモリが不足する",
      "カーネルが実行されない"
    ],
    answer: 1,
    explanation: "CUDAの最小実行単位は1ワープ（32スレッド）であるため，要素数が32の倍数でない場合，最後のワープの一部スレッドがアイドル状態となり利用されない．"
  },
  {
    question: "要素数が2の累乗でない場合にグリッドサイズ（ブロック数）を計算する正しい式はどれか？",
    options: [
      "N / threadsPerBlock",
    ...
    answer: 2,
    explanation: "`(N + threadsPerBlock - 1) / threadsPerBlock`は切り上げ除算であり，すべての要素をカバーするために必要なブロック数を正しく計算する．`N / threadsPerBlock`では端数分の要素が処理されない．"
  },
  {
    question: "要素数が2の累乗でないベクトルを処理するカーネル内で不可欠な処理は何か？",
    options: [
      "cudaDeviceSynchronize()の呼び出し",
      "共有メモリの使用",
      "if (i < n) による境界チェック",
      "ワープIDの計算"
    ],
    answer: 2,
    explanation: "`if (i < n)`による境界チェックは，余剰スレッドが不正なメモリアクセスを行わないために不可欠である．ブロック数の切り上げにより実際の要素数以上のスレッドが起動されるため，範囲外アクセスを防止する必要がある．"
  }
]} />
