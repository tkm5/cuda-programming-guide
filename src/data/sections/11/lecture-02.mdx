...
<Quiz questions={[
  {
    question: "shared memoryを使った行列積最適化において，__syncthreads()が必要な箇所はどこか？",
    options: [
      "ロード完了後の1箇所のみ",
      "演算完了後の1箇所のみ",
      "ロード完了後と演算完了後の2箇所",
    ...
    answer: 2,
    explanation: "__syncthreads()は，グローバルメモリからshared memoryへのロード完了後（全スレッドのロードが完了するまで待機）と，タイル内の演算完了後（次のタイルをロードする前に全スレッドの演算完了を保証）の2箇所で必要である．"
  },
  {
    question: "タイルサイズ64×64が行列積のCUDAカーネルで使用できない理由は何か？",
    options: [
      "shared memoryの容量を超えるため",
      "ブロックあたり最大スレッド数1024を超える（64×64 = 4096スレッド）ため",
      "グローバルメモリのアライメント制約に違反するため",
      "ワープサイズ32の倍数でないため"
    ],
    answer: 1,
    explanation: "タイルサイズ64×64の場合，1ブロックあたり64 × 64 = 4096スレッドが必要となるが，CUDAではブロックあたり最大1024スレッドの制限があるため実行できない．"
  },
  {
    question: "グローバルメモリとshared memoryのレイテンシの差はおおよそどの程度か？",
    options: [
      "約2倍（グローバル: 40サイクル，shared: 20サイクル）",
      "約5倍（グローバル: 100サイクル，shared: 20サイクル）",
      "約15倍（グローバル: 300サイクル，shared: 20サイクル）",
      "約50倍（グローバル: 1000サイクル，shared: 20サイクル）"
    ],
    answer: 2,
    explanation: "グローバルメモリのレイテンシは約300サイクル，shared memoryのレイテンシは約20〜22サイクルであり，約15倍の差がある．この差がshared memoryを使った最適化の根拠となる．"
  }
]} />
