...
<Quiz questions={[
  {
    question: "GPU上のNaive行列積実装において，CPUの3重ループのうちGPUのスレッド/ブロックIDで置き換えられるのはどれか？",
    options: [
      "k次元のループのみ",
      "行と列の外側2つのループ",
      "3つのループすべて",
    ...
    answer: 1,
    explanation: "GPUのNaive実装では，外側の行ループと列ループをblockIdx/threadIdxで置き換え，k次元のループのみカーネル内に残す．各スレッドがC行列の1要素を担当する．"
  },
  {
    question: "行列積 C = A × B において，AがM×K，BがK×Nの場合，結果行列Cのサイズはいくつか？",
    options: [
      "M×K",
      "K×N",
      "M×N",
    ...
    answer: 2,
    explanation: "行列積ではAの行数（M）とBの列数（N）が結果行列Cの次元を決定する．したがって，CのサイズはM×Nとなる．"
  },
  {
    question: "Naive GPU行列積のブロックサイズを16×16に設定した場合，1ブロックあたりのスレッド数はいくつか？",
    options: [
      "16",
      "32",
      "128",
    ...
    answer: 3,
    explanation: "ブロックサイズが16×16の場合，1ブロックあたりのスレッド数は16 × 16 = 256スレッドとなる．"
  }
]} />
