...
```mermaid
graph TD
    A["カーネル起動 blocks, threads"] --> B{"threads が 1024以下?"}
    B -->|"はい"| C["正常に実行"]
    B -->|"いいえ"| D["実行失敗"]
...
<Quiz questions={[
  {
    question: "GPU上で実行されるカーネル関数を宣言するために使用する修飾子はどれか？",
    options: [
      "__device__",
      "__host__",
      "__global__",
    ...
    answer: 2,
    explanation: "`__global__`修飾子はGPU上で実行されるカーネル関数を宣言するために使用される．`__device__`はGPUから呼び出されるデバイス関数，`__host__`はCPU上で実行される関数を示す．"
  },
  {
    question: "Ampereアーキテクチャにおけるブロックあたりの最大スレッド数はいくつか？",
    options: [
      "256",
      "512",
      "1024",
    ...
    answer: 2,
    explanation: "Ampereアーキテクチャではブロックあたり最大1024スレッドである．これを超えるスレッドが必要な場合はブロック数を増やして対応する．"
  },
  {
    question: "カーネル起動パラメータ `hello_kernel<<<2, 8>>>();` で起動されるスレッドの総数はいくつか？",
    options: [
      "2",
      "8",
      "10",
    ...
    answer: 3,
    explanation: "ブロック数2 x ブロックあたりのスレッド数8 = 合計16スレッドが起動される．`<<<ブロック数, スレッド数>>>`の形式で指定する．"
  }
]} />
