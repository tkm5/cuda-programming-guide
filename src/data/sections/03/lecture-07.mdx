...
<Quiz questions={[
  {
    question: "複数ブロック構成でグローバルインデックスを計算する正しい式はどれか？",
    options: [
    ...
    answer: 2,
    explanation: "グローバルインデックスは`threadIdx.x + blockIdx.x * blockDim.x`で計算する．`blockIdx.x * blockDim.x`がブロックの開始位置を表し，そこにブロック内のスレッドIDを加算する．"
  },
  {
    question: "A100 GPU（108 SM）で2ブロック x 1024スレッドの構成を使用した場合，GPU使用率はおよそどの程度か？",
    options: [
      "約2%",
      "約25%",
      "約50%",
    ...
    answer: 0,
    explanation: "2ブロックは2つのSMでのみ実行される．A100は108個のSMを持つため，GPU使用率は2/108 = 約2%となり，ほとんどのSMがアイドル状態になる．"
  },
  {
    question: "CUDAにおけるブロックあたりの最小スレッド数はいくつか？",
    options: [
      "1",
      "16",
      "32",
    ...
    answer: 2,
    explanation: "ブロックあたりの最小スレッド数は32（1ワープ）である．GPUの最小実行単位がワープ（32スレッド）であるため，これ未満のスレッド数を設定しても32スレッド分のリソースが消費される．"
  }
]} />
