...
<Quiz questions={[
  {
    question: "CUDAアプリケーションの基本ステップとして正しい順序はどれか？",
    options: [
      "カーネル実行 → メモリ確保 → データ転送 → メモリ解放",
      "メモリ確保 → 初期化 → CPU→GPU転送 → カーネル実行 → GPU→CPU転送 → メモリ解放",
      "初期化 → メモリ確保 → カーネル実行 → データ転送 → メモリ解放",
      "メモリ確保 → カーネル実行 → 初期化 → データ転送 → メモリ解放"
    ],
    answer: 1,
    explanation: "CUDAアプリケーションは「メモリ確保 → 初期化 → CPU→GPU転送 → カーネル実行 → GPU→CPU転送 → メモリ解放」の6ステップで構成される．"
  },
  {
    question: "GPU側のメモリを確保するために使用するCUDA関数はどれか？",
    options: [
      "malloc",
      "cudaAlloc",
      "cudaMalloc",
    ...
    answer: 2,
    explanation: "`cudaMalloc`はGPU側（デバイス側）のメモリを確保するために使用される．CPU側のメモリ確保には通常の`malloc`を使用する．"
  },
  {
    question: "GPUからCPUへデータを転送する際に指定する転送方向フラグはどれか？",
    options: [
    ...
    answer: 1,
    explanation: "GPU（Device）からCPU（Host）へデータを転送する場合は`cudaMemcpyDeviceToHost`を指定する．逆方向のCPUからGPUへは`cudaMemcpyHostToDevice`を使用する．"
  }
]} />
