...
<Quiz questions={[
  {
    question: "CUDAプログラムをコンパイルするために使用するコマンドは何か？",
    options: [
      "gcc",
      "g++",
      "nvcc",
    ...
    answer: 2,
    explanation: "`nvcc`はNVIDIAが提供するCUDAコンパイラであり，CUDAソースファイル（.cu）をコンパイルして実行ファイルを生成する．"
  },
  {
    question: "cudaDeviceSynchronize()の役割は何か？",
    options: [
      "GPUメモリを解放する",
      "カーネルを起動する",
      "CPUがGPUの処理完了を待つ",
    ...
    answer: 2,
    explanation: "`cudaDeviceSynchronize()`はCPUがGPUカーネルの実行完了を待つために使用される．これがないと，GPUの処理が完了する前にCPUが次の処理に進んでしまい，出力が表示されない場合がある．"
  }
]} />
