...
<Quiz questions={[
  {
    question: "SIMT（Single Instruction Multiple Thread）方式において，1ワープは何スレッドで構成されますか？",
    options: [
      "16スレッド",
      "32スレッド",
      "64スレッド",
    ...
    answer: 1,
    explanation: "ワープは32スレッドのグループで構成されます．SIMT方式では，全スレッドが同じ命令を異なるデータに対して同時実行します．例えばベクトル加算では，各スレッドが同じ「加算」命令を異なる要素に対して実行します．"
  },
  {
    question: "Voltaアーキテクチャで初めて導入された画期的な機能はどれですか？",
    options: [
      "L2キャッシュ",
      "NVLink",
      "Tensor Core",
    ...
    answer: 2,
    explanation: "Tensor CoreはVoltaアーキテクチャ（2017年）で初めて導入されました．行列演算専用のユニットであり，スループットを最大12倍向上させ，AIアプリケーションの性能を飛躍的に改善しました．"
  },
  {
    question: "VoltaのSM（Streaming Multiprocessor）はいくつのパーティションに分割されていますか？",
    options: [
      "2つ",
      "4つ",
      "8つ",
    ...
    answer: 1,
    explanation: "VoltaのSMは4つのパーティションに分割されています．各パーティションにはL0命令キャッシュ，ワープスケジューラ，ディスパッチャ，レジスタファイル，16個の単精度コア，16個の整数コア，Tensor Core，特殊関数ユニットが含まれます．"
  }
]} />
