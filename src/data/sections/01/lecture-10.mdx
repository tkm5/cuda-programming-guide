...
<Quiz questions={[
  {
    question: "Compute Capabilityのメジャー番号（ドットの前の数字）は何を示しますか？",
    options: [
      "GPUの製造年を示す",
      "対応するCUDAバージョンを示す",
      "主要なアーキテクチャの変更を示す",
    ...
    answer: 2,
    explanation: "Compute Capabilityのメジャー番号（ドットの前の数字）は主要なアーキテクチャの変更を示します．マイナー番号（ドットの後の数字）は軽微な改善や拡張を示します．"
  },
  {
    question: "Tensor Coreを利用するために必要な最低限のCompute Capabilityはいくつですか？",
    options: [
      "CC 5.0",
      "CC 6.0",
      "CC 7.0",
    ...
    answer: 2,
    explanation: "Tensor CoreはCC 7.0（Voltaアーキテクチャ）以上でのみ利用可能です．CC 5.0では半精度演算もTensor Coreも対応していません．"
  },
  {
    question: "HopperアーキテクチャのGPUで使用できるCUDAツールキットの最小バージョンはどれですか？",
    options: [
      "CUDA 6.5",
      "CUDA 10.0",
      "CUDA 11.8",
    ...
    answer: 2,
    explanation: "Hopperアーキテクチャ（CC 9.x）では，最小CUDAバージョンが11.8です．これより古いCUDAバージョンを使用すると互換性エラーが発生します．"
  }
]} />
