...
<Quiz questions={[
  {
    question: "Nsight IntegrationとNsight Computeの役割の違いとして正しいものはどれか？",
    options: [
      "どちらもプロファイリング専用ツールであり，デバッグ機能はない",
      "Nsight Integrationはブレークポイントを使ったデバッグツール，Nsight ComputeはSM利用率やL1/L2ヒット率などの性能メトリクスを収集するプロファイリングツール",
      "Nsight ComputeはCPUコードのデバッグ，Nsight IntegrationはGPUコードのプロファイリングに使用する",
      "Nsight IntegrationはLinux専用，Nsight ComputeはWindows専用のツールである"
    ],
    answer: 1,
    explanation: "Nsight Integrationはブレークポイントの設定や変数の状態確認などのデバッグ機能を提供する．Nsight Computeは実行時間，SM利用率，オキュパンシ，L1/L2ヒット率などの性能メトリクスを収集するプロファイリングツールである．"
  },
  {
    question: "Visual Studioのメモリウィンドウでデバッグ中にステップ実行した際，新たに計算された要素はどのように表示されるか？",
    options: [
      "太字で表示される",
      "赤色でハイライトされる",
      "点滅して表示される",
    ...
    answer: 1,
    explanation: "デバッグ中にステップ実行するたびに，新たに計算された要素はメモリウィンドウ上で赤色にハイライトされて表示される．これにより，各ステップでどのメモリ位置が更新されたかを視覚的に確認できる．"
  }
]} />
