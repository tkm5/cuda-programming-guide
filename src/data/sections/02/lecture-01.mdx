...
<Quiz questions={[
  {
    question: "CUDAツールキットにおいて，高レベルのCUDAコードをGPUが理解できる形式に変換するコンパイラは何ですか？",
    options: [
      "gcc",
      "cuda-gdb",
      "nvcc",
    ...
    answer: 2,
    explanation: "nvcc（NVIDIA CUDA Compiler）はCUDA開発の中心にあるコンパイラです．高レベルのCUDAコードをPTXコードに変換し，様々なNVIDIAアーキテクチャと互換性のある実行ファイルを生成します．"
  },
  {
    question: "GPU上にメモリを確保するためのCUDAランタイムAPI関数はどれですか？",
    options: [
      "cudaMemcpy",
      "cudaMalloc",
      "cuBLAS",
    ...
    answer: 1,
    explanation: "cudaMallocはGPU上にメモリを確保するための関数です．C言語のmallocに相当しますが，GPU側のメモリを対象とします．cudaMemcpyはホスト（CPU）とデバイス（GPU）間でデータを転送する関数です．"
  },
  {
    question: "ディープラーニング向けニューラルネットワーク高速化ライブラリはどれですか？",
    options: [
      "cuBLAS",
      "cuFFT",
      "cuRAND",
    ...
    answer: 3,
    explanation: "cuDNNはディープラーニング向けニューラルネットワーク高速化ライブラリです．cuBLASは線形代数演算用，cuFFTは高速フーリエ変換用，cuRANDは乱数生成用のライブラリです．"
  }
]} />
