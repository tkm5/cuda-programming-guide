...
flowchart TD
    ...
    K3 --> |"最終結果: 1個"| R["input#91;0#93;に格納"]
```
...
<Quiz questions={[
  {
    question: "100万要素のベクトルに対し blockSize=256 でリダクションを実行する場合，最終結果を得るために必要なカーネル起動回数は何回か？",
    options: [
      "1回",
      "2回",
      "3回",
    ...
    answer: 2,
    explanation: "100万要素では gridSize=4096 となる．カーネル1（grid=4096）で4096個の部分和を生成し，カーネル2（grid=16）で16個の部分和にまとめ，カーネル3（grid=1）で最終結果を得る．合計3回のカーネル起動が必要となる．"
  },
  {
    question: "ベクトルリダクションのプロファイリングで，L1キャッシュヒット率が約91%と高い値を示す理由として正しいものはどれか？",
    options: [
      "ベクトルサイズがL1キャッシュに完全に収まるため",
      "リダクションの各ステップで同じ要素を複数回再利用するため",
      "グローバルメモリへのアクセスがすべてコアレッシングされるため",
      "共有メモリを使用しているため"
    ],
    answer: 1,
    explanation: "ベクトルリダクションでは，各ステップで前のステップの結果を再利用する．同じメモリ位置に対して複数回のアクセスが発生するため，L1キャッシュにヒットするデータが増え，ヒット率が高くなる．"
  }
]} />
