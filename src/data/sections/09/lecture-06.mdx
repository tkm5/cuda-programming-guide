...
<Quiz questions={[
  {
    question: "共有メモリを導入することで，グローバルメモリへの書き込みデータ量はどの程度削減されたか？",
    options: [
      "18-19MBから10MBに削減",
      "18-19MBから5MBに削減",
      "18-19MBから1MB未満に削減",
    ...
    answer: 2,
    explanation: "共有メモリを導入することで，リダクションの中間ステップの書き込みが共有メモリ上で完結し，グローバルメモリへの書き込みは最終結果のみとなる．これにより書き込みデータ量が18-19MBから1MB未満に大幅に削減される．"
  },
  {
    question: "ループアンローリングでストライド32未満の反復を展開する理由として正しいものはどれか？",
    options: [
      "ストライド32未満ではメモリアクセスが非効率になるため",
      "ストライド32未満は1ワープ内の処理であり，__syncthreads() の同期オーバーヘッドを排除できるため",
      "ストライド32未満ではバンクコンフリクトが発生するため",
      "コンパイラがストライド32未満のループを最適化できないため"
    ],
    answer: 1,
    explanation: "ストライド32未満の処理は1ワープ（32スレッド）内で完結する．ワープ内のスレッドはロックステップで実行されるため，__syncthreads() による同期が不要となる．アンローリングにより同期オーバーヘッドを排除し，約10%の性能向上を達成する．"
  },
  {
    question: "forループの方向を大きいストライドから小さいストライドへ（stride >>= 1）にすることの利点はどれか？",
    options: [
      "コードが読みやすくなる",
      "共有メモリのバンクコンフリクトがゼロになる",
      "スレッド使用率が向上する",
      "カーネル起動回数が減少する"
    ],
    answer: 1,
    explanation: "forループを大きいストライドから開始する方向にすることで，共有メモリのバンクコンフリクトがゼロになる．逆方向（stride=1から開始）では数十のコンフリクトが発生し，実行時間が約115マイクロ秒に悪化する．"
  }
]} />
