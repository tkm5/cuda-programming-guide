...
<Quiz questions={[
  {
    question: "Nsight Computeでプロファイリングを実行するコマンドラインツールは何か？",
    options: [
      "nvprof",
      "nsys",
      "ncu",
    ...
    answer: 2,
    explanation: "`ncu`はNsight Computeのコマンドラインインターフェースである．`nvprof`は非推奨，`nsys`はNsight Systemsのコマンドラインツールである．"
  },
  {
    question: "Windows環境（WSL使用時）でncuコマンドがエラーになる主な原因は何か？",
    options: [
      "CUDAドライバがインストールされていない",
      "GPUパフォーマンスカウンターへのアクセス権限がない",
      "nvccコンパイラのバージョンが古い",
      "WSLがGPUをサポートしていない"
    ],
    answer: 1,
    explanation: "GPUドライバのインストール時にパフォーマンスカウンターへのアクセスが管理者ユーザーのみに制限されるため，一般ユーザーではプロファイリングに失敗する．NVIDIA Control Panelで全ユーザーへのアクセスを許可することで解決できる．"
  }
]} />
