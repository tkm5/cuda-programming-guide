...
<Quiz questions={[
  {
    question: "Nsight Computeのメトリクス名の構造として正しいものはどれか？",
    options: [
      "メトリクス名.ハードウェアユニット.集計方法",
      "集計方法__メトリクス名.ハードウェアユニット",
      "ハードウェアユニット__メトリクス名.集計方法",
      "メトリクス名__集計方法.ハードウェアユニット"
    ],
    answer: 2,
    explanation: "メトリクス名は「ハードウェアユニット__メトリクス名.集計方法」という体系的な命名規則に従う．例: `l1tex__t_sector_hit_rate.pct`は，L1キャッシュ（l1tex）のセクターヒット率（t_sector_hit_rate）をパーセント（.pct）で表示する．"
  },
  {
    question: "ncuコマンドで特定のセクションのみを表示するためのオプションはどれか？",
    options: [
      "--filter",
      "--section",
      "--select",
    ...
    answer: 1,
    explanation: "`--section`オプションで特定のセクション（例: LaunchStats，WarpStateStats）のみを表示できる．デフォルト実行では4つのセクションが表示される．"
  },
  {
    question: "メトリクスの集計方法（サフィックス）のうち，全SMの合計値を表すものはどれか？",
    options: [
      ".avg",
      ".max",
      ".min",
    ...
    answer: 3,
    explanation: "`.sum`はGPU全体の合計値を表す．`.avg`は全SMの平均値，`.max`は最大値，`.min`は最小値である．`.sum`は最大値 x SM数で計算される．"
  }
]} />
