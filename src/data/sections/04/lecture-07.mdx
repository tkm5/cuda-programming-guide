...
<Quiz questions={[
  {
    question: "Nsight ComputeとNsight Systemsの違いとして正しいものはどれか？",
    options: [
      "Nsight Computeはシステム全体の分析，Nsight Systemsはカーネル単位の分析",
      "Nsight Computeはカーネル単位の詳細分析，Nsight Systemsはシステム全体の分析",
      "両方とも同じ機能を提供するが，対応OSが異なる",
      "Nsight Computeはデバッグ専用，Nsight Systemsはプロファイリング専用"
    ],
    answer: 1,
    explanation: "Nsight Computeは個々のカーネルの詳細分析に特化し，100,000以上のメトリクスを提供する．一方，Nsight Systemsはアプリケーション全体のシステムレベル分析に適しており，CPU-GPU間のインタラクションなどを俯瞰する．"
  },
  {
    question: "NVIDIAのプロファイリングツールのうち，非推奨（使用すべきでない）ツールはどれか？",
    options: [
      "Nsight Systems",
      "Nsight Compute",
      "CUDA-GDB",
    ...
    answer: 3,
    explanation: "`nvprof`は非推奨となっている．代わりにNsight Systems（システムレベル分析）やNsight Compute（カーネルレベル分析）を使用することが推奨される．"
  },
  {
    question: "Nsight Computeが提供するメトリクスの数はおよそいくつか？",
    options: [
      "約1,000",
      "約10,000",
      "約100,000以上",
    ...
    answer: 2,
    explanation: "Nsight Computeは100,000以上のメトリクスを提供し，グローバルメモリ，キャッシュ，SM内の各ユニットに至るまで，非常に詳細なカーネルレベルの分析が可能である．"
  }
]} />
